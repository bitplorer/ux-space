...
| [soft_surface/](soft_surface/) | Soft-surface leftover tour: Soft 2 SHAPES, Soft 3 camera/light, Soft 4 transform + `material.type=basic`, Soft 5 awareness (day-1 stays threejs; canvas is swap-proof only), Soft 6 `pickable` + Cap-gated `pick`, Soft 7 camera `orbit` / `pan` / `zoom`, Soft 8 `material.type=standard`, Soft 9 `.gltf` / `.texture` + `material.map` |
